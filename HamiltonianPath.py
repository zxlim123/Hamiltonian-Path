...
def pairing (alph,nodes) : # pair all node
    ...
    edges = math.ceil(len(pairsSet)/3 - round(len(pairsSet)*random.uniform(0.0,0.1))) #50-70%
    ...
    if len(subnode) != len(nodes): #missing node direct end
        ...
        if len(marker) == len(nodes):
            ...
            if len(marker) == len(nodes):
                ...
                # for loop 50 time
                # marker = []
                # every time change the subnode order if can edges order also
                #if len marker == len node break end
                #end of loop push marker to candidate
                
                #after for loop check best candidate
                # print out then end
